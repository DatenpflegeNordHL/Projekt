...
function reconcileUsage() {
  ...
  if (!runDirectory) throw new Error("CODEXLOOPER_RUN_DIR is required for Terra usage reconciliation");
...
try {
  ...
  process.stderr.write(
    `CODEXLOOPER_TERRA_RUNTIME_BLOCK: ${error.code || "CODEXLOOPER_RUNTIME_FAILED"}: ${error.message}\n`,
  );
  process.exitCode = 1;
}
