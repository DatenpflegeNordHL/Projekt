...
- The currently installed runner does not forward `CODEXLOOPER_CRG_COMMAND`; this implementation run cannot honestly claim a live CRG advisory result.
- The upstream interface is pinned to `tirth8205/code-review-graph` version `2.3.6`, repository commit `6ce25b4e53f9df397f5136e86a59e17c02a610fe`.
...
- The upstream installer writes MCP configuration, hooks and instruction files. CodexLooper must never invoke it.
...
1. Version check:
   ...
   - Required normalized version: exactly `2.3.6`.
...
3. Read-only change analysis:
   ...
   - Do not pass `--brief`; JSON output is required for deterministic parsing.
...
- `DO_NOT_TRACK=1`
- `NO_COLOR=1`
- preserve only the minimum non-secret variables required for the executable and `/usr/bin/git` discovery.
...
- CRG is optional and disabled unless `CODEXLOOPER_CRG_COMMAND` contains one exact absolute executable path.
...
- Pass the optional CRG executable and run-start Git SHA from `src/run.mjs` into the controlled Ralphex environment.
...
- Do not add CRG as an npm dependency.
- Defer the first live CRG proof to WP6B after re-bootstrap under Node.js 22.
...
- `node --check src/run.mjs && node --check src/git-supervisor.mjs && node --check bin/sol-review.mjs && node --check scripts/install.mjs && node --check scripts/preflight.mjs`
...
- `node --test tests/install.test.mjs tests/run.test.mjs tests/sol-review.test.mjs`
- `npm run check`
- `git diff --check`
...
Implement only the standalone adapter layer and its direct tests. Add `src/code-review-graph.mjs`, add `tests/code-review-graph.test.mjs`, and update `package.json` only so the repository-wide check includes the new source. Follow every exact normalized-result, executable-fixture, private-runtime-tree and mocked-process rule above. Cover disabled mode, exact v2.3.6 acceptance, wrong version, unsafe path, private path rejection, symlink rejection, timeout, non-zero exit, excessive stdout/stderr, malformed JSON, credential redaction, private data paths and exact argument arrays. Do not integrate the adapter into the runner or Sol yet.

- [ ] Return the exact Task 1 unified diff and mark only this checkbox complete.

### Task 2: Integrate controlled runner environment, validation PATH and receipt metadata

Integrate the adapter contract into `src/run.mjs`, `scripts/install.mjs` and `scripts/preflight.mjs` as required. Forward `CODEXLOOPER_CRG_COMMAND` only when configured, forward the exact run-start SHA under a dedicated controlled variable, preserve disabled legacy behavior and expose only secret-free normalized receipt metadata. Fix host validation so it executes with the runner's inherited Node.js 22 PATH rather than replacing it through a login shell; do not weaken the validation command allowlist or environment filtering. Modify `src/git-supervisor.mjs`, add or extend `tests/git-supervisor.test.mjs`, and extend `tests/run.test.mjs` and `tests/install.test.mjs`. Do not modify `bin/sol-review.mjs` in this task.
